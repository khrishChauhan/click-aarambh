...
export default function AdminReelsManager({ onShowToast }: AdminReelsManagerProps) {
  ...
  const handleDeleteReel = async (id: string, title: string) => {
    if (!window.confirm(`Are you sure you want to remove "${title}" from the homepage carousel?`)) {
      return;
    }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            <AnimatePresence mode="popLayout">
              {reels.map((reel) => {
                ...
                const thumb =
                  reel.thumbnail && !reel.thumbnail.startsWith("data:image")
                    ? reel.thumbnail
                ...
                return (
                  ...
                  >
                    {/* Thumbnail Media Box */}
                    <div className="relative aspect-[9/16] w-full overflow-hidden bg-[#0D2E26]/5 max-h-[300px]">
                    ...
                    <div className="p-4 flex flex-col justify-between flex-1">
                      <div>
                        ...
                        <p className="font-mono text-[11px] text-[#4B635D] mb-3">
