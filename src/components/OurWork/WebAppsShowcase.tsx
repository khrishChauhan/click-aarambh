...
function WebAppCard({ app }: { app: typeof WEB_APPS[0] }) {
  return (
    <a
      ...
      className="webapp-card relative block rounded-2xl flex-none shrink-0 group cursor-pointer border border-[#0D2E26]/10 overflow-hidden bg-white w-[calc(min(100vw,1400px)-48px)] md:w-[calc((min(100vw,1400px)-72px)/2)] lg:w-full aspect-[4/3] transition-all duration-500 hover:scale-[1.02]"
      style={{
...
export default function WebAppsShowcase() {
  ...
  return (
    <div className="container relative z-10 w-full max-w-[1400px] mx-auto px-6 mt-16 md:mt-32">
      ...
      >
        <div className="flex lg:grid lg:grid-cols-3 gap-6 w-max lg:w-full items-center px-6 md:px-0 after:content-[''] after:w-px md:after:hidden">
